...
def select_levels(t0,T,M_t,eps,F,strategy=1,cold_start=True,N=100,boundary=None):
    ...
    '''
    if strategy==1:
        dt_0 = np.minimum((eps/10)**2,0.025)
        levels = np.array([dt_0,dt_0/M_t])
...
def ml(e2,Q,t0,T,M_t,eps,M,r,F,N_warm=40,boundary=None,strategy=1,alpha=None,beta=None,gamma=None):
    ...
    while True:
        ...
        if strategy==1:
            levels = np.append(levels[0]/(M_t**(L-1)))
        else:
...
def ml_test(N,N0,dt_list,E2,Q,t0,T,M_t,eps,M,r,F,logfile,boundary=None,strategy=None):
    ''''
    ...
    for e2 in E2:
        E,_,C,N,_ = ml(e2,Q,t0,T,M_t,eps,M,r,F,N0,boundary=boundary,strategy=strategy,alpha=alpha)
