...
# @njit(nogil=True)
def select_levels(t0,T,M_t,eps,r,F,strategy=1,N=100,boundary=None):
    if strategy==1:
        dt_0 = np.minimum((eps)**2/r(np.array([0])),T-t0)
        levels = dt_0/M_t**np.arange(0,2)
...
def ml_test(N,N0,dt_list,E2,Q,t0,T,M_t,eps,M,r,F,logfile,boundary=None,strategy=1,convergence=True,complexity=True,rev=False,diff=False,v_ms=1,std=False):
    ...
    if complexity:
        ...
        if convergence:
            ...
            c4 = np.mean(Cost*E2[start:end]/np.log(E2[start:end]))
            c4 *= 2
            ax2.plot(E2[start:end],c4*np.log(E2[start:end])/E2[start:end],label= r'$\mathcal{O}(\log{E}^2E^{-2})$')
            ax1.set_yscale('log')
