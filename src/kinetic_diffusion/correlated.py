...
'''Method for correlating paths of different levels in the Multilevel Monte Carlo
    method'''
def correlated(dt_f,dt_c,x0,v0,v_l1_next,t0,T,mu,sigma,M,R,SC,R_anti = None,dR=None,boundary=None):
    '''
    dt_f: time step for fine level
    dt_c: time step for coarse level
    x0: initial position
    v0: initial velocity
    dR: Derivative of collision rate
    '''
    n = len(x0)
...
def integral_of_R(R,t_c,t_f,x,v,R_anti):
    ...
    if R_anti is None:
        ...
        # #Check that coarse path is ahead of fine path
        index = np.argwhere(t_c>t_f).flatten()
